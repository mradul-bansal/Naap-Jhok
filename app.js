...
app.get('/', (req, res) => {
  ...
  const customerName = req.session.customerName || null;
...
// Customer Dashboard Route (Update to pass customer name)
app.get("/dashboard/:id", async (req, res) => {
  try {
    const customer = await Customer.findById(req.params.id);

    if (customer) {
      res.render("listings/index.ejs", { customerName: customer.email }); // Pass the customer name
    } else {
...
// // Helper function to calculate order total (you can adjust this logic)
// function calculateTotal(item, quantity) {
//   // For example, you can set a flat price per item (e.g., 50 units)
